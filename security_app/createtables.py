...
TABLES = {
    'tenant': {
        'prefix': 'tenant-sec',
        'env_var': 'TENANT_TABLE_NAME',
        'hash_key': 'tenant_id'
    },
    'usertenants': {
        'prefix': 'usertenants-sec',
        'env_var': 'USERTENANTS_TABLE_NAME',
...
def main():
    ...
    parser.add_argument(
        ...
        '--table-type',
        default='users',
        choices=['user', 'tenant', 'usertenants'],
        help='Specify which type to create')
    args = parser.parse_args()
    table_config = TABLES[args.table_type]
    table_name = create_table(table_config['prefix'], table_config['hash_key'],
